...
drop procedure if exists [dbo].[sp_process_salesorder]
GO

-- create a loop whitout using cursosrs

CREATE PROCEDURE [dbo].[sp_process_salesorder] 
AS
BEGIN
	...
	begin
		...
		-- add date
		exec [dbo].[sp_add_date]
			@salesorder_date,
			@date_key OUTPUT;
		if ( @date_key  is NULL ) begin set @result = 004; goto next; end
		...
		if ( @individual_customer_key  is NULL ) begin set @result = 104; goto next; end
		...
		exec [dbo].[sp_add_employee]
			...
			@employee_name  ,
			@employee_key  OUTPUT; 
		if ( @employee_key  is NULL ) begin set @result = 204; goto next; end

		-- add store
		exec [dbo].[sp_add_store]
				@store_id, @store_name, @store_year_open, @store_square_feet,
				@store_specialty, @store_business_type, @store_city, @store_state_province,
				@store_country,
				@store_key OUTPUT; 
		if ( @store_key  is NULL ) begin set @result = 304; goto next; end

		-- add territory
		exec [dbo].[sp_add_territory]
				@territory_id, @territory_name, @territory_country_region_name,
				@territory_group_name,
				@territory_key OUTPUT; 
		if ( @territory_key  is NULL ) begin set @result = 404; goto next; end
...
next:	--------------------- finish processing
		...
		set [salesorder_processed] = 1,
			[salesorder_processed_result] = @result
		where [record_key] = @record_key;
		
		select @record_key record_key, @result result
